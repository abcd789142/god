local function isPlayersAvailable()
...
if not isPlayersAvailable() then
    warn("[WindUI] Warning: Running in an environment where Players service is unavailable. Some features may not work.")
end
...
local ag = ab and ab.LocalPlayer or nil
if not ag then
    warn("[WindUI] Players service or LocalPlayer is not available. Running in a restricted environment.")
    ag = {Name = "Unknown", DisplayName = "Unknown"} -- 提供默认值
end
...
local j = ag and ag.Name or "Unknown" -- 确保 ag 存在时才访问 Name

if e.KeySystem then
    h = false
    if e.KeySystem.SaveKey and e.Folder and ag then -- 添加 ag 检查

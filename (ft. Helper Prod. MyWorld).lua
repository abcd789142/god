...
scriptName.Text = "ControlerScript V2 Edu:1ELvin1 and YA BO (Prod. MyWorld)"
...
local viewbutton = {Text = "jian shi "}

local playersframe = makeFrame(scrollingFrame, "wan jia", Color3.fromRGB(12, 59, 100))
...
local function addbtn(parent, plr)
	...
	playerbutton.MouseButton1Click:Connect(function()
		playercframe.framelabel.Text = "wan jia:" .. playerbutton.Text
		currentplayer = plr
		playercframe.Visible = true
		playerframef.Visible = false
		viewbutton.Text = ((viewedPlayer == plr) and "tui chu jian shi") or "jian shi"
	end)
end

local function unview()
    viewedPlayer = nil
    viewbutton.Text = "jian shi"
...
makeplrbutton("chuan song").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c and c.Parent then
...
viewbutton = makeplrbutton("jian shi")
viewbutton.MouseButton1Click:Connect(function()
    if viewedPlayer == currentplayer then
        unview()
    else
	    viewedPlayer = currentplayer
	    viewbutton.Text = "tui chu"
	end
end)

local cbringb = makeplrbutton("zi zhui !!!!!!!!!")
...
makeplrbutton("sha diao").MouseButton1Click:Connect(function()
	local plr = currentplayer
    local c1 = plr.Character
    if not (c1 and c1.Parent) then
        notify("target character not found")
...
makeplrbutton("fu jia").MouseButton1Click:Connect(function()
	return attach(currentplayer.Character) and notify("attached to " .. currentplayer.Name)
end)

makeplrbutton("ta de hao you").MouseButton1Click:Connect(function()
...
local function makeflingbutton(partname)
	makeplrbutton("shuai fei ta (" .. partname .. ")").MouseButton1Click:Connect(function()
		local c = lp.Character
		if c and c.Parent then
...
makeflingbutton("tou")
makeflingbutton("shou")
makeflingbutton("shen ti")

local graphicsframe = makeFrame(scrollingFrame, "tu xing", Color3.fromRGB(84, 45, 162))
...
makegraphicsbutton("shan chu tie tu").MouseButton1Click:Connect(function()
	lig.FogStart = 9e9
	lig.FogEnd = 9e9
end)
...
setupremove(makegraphicsbutton("shu chu te xiao"), "Atmosphere")
setupremove(makegraphicsbutton("xiao chu mo hu"), "BlurEffect")
setupremove(makegraphicsbutton("shan chu tie tu"), "Decal")
setupremove(makegraphicsbutton("mo ren tian kong"), "Sky")
setupremove(makegraphicsbutton("qu chu yang guang"), "SunRaysEffect")
setupremove(makegraphicsbutton("shan chu li zi xiao guo"), "ParticleEmitter")
setupremove(makegraphicsbutton("shan chu yan se jiu zheng"), "ColorCorrectionEffect")

local cfly = nil
local fhrp = nil
local flyspeed = 1

local positionsframe = makeFrame(scrollingFrame, "wei zhi", Color3.fromRGB(162, 108, 42))
...
addpositionbutton.Text = "+tian jia"
...
local charframe = makeFrame(scrollingFrame, "xiu gai shu ju xiao xin shi yong!!!!!", Color3.fromRGB(99, 15, 15))
...
makecharbutton("chong sheng").MouseButton1Click:Connect(function()
	if not loopr then
		reset(true)
	end
end)
makecharbutton("chong zhi").MouseButton1Click:Connect(function()
	reset(false)
end)
local looprb = makecharbutton("xun huan chong sheng")
spawn(function()
...
looprb.MouseButton1Click:Connect(function()
	loopr = not loopr
	looprb.Text = "xun huan chuang sheng" .. ((loopr and " (kai)") or "")
end)
makecharbutton("shan chu wang ge").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c and c.Parent then
...
makecharbutton("duan kai lian jie").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c and c.Parent then
		c:BreakJoints()
...
local noclipb = makecharbutton("chuan qiang")
local antiflingb = makecharbutton("bu zhi dao")
local noclip = false
local antifling = false
noclipb.MouseButton1Click:Connect(function()
	noclip = not noclip
	noclipb.Text = "chuan qiang" .. ((noclip and " (kai)") or "")
end)
antiflingb.MouseButton1Click:Connect(function()
	antifling = not antifling
	antiflingb.Text = "jia kong" .. ((antifling and " (kai)") or "")
end)
local fakevoidb = makecharbutton("yin shen")
fakevoidb.MouseButton1Click:Connect(function()
    if fakevoidp then
        fakevoidp = nil
        fakevoidb.Text = "yin shen"
    else
        local c = lp.Character
        if not (c and c.Parent) then
            notify("character not found")
...
makecharbutton("fang kuai gong ju").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c and c.Parent then
		local tool = c:FindFirstChildOfClass("Tool")
		if tool then
...
local infjumpb = makecharbutton("lian tiao")
local infjump = false
local con = nil
...
infjumpb.MouseButton1Click:Connect(function()
	infjump = not infjump
	infjumpb.Text = "lian tiao" .. ((infjump and "(开)") or "")
end)
makecharbutton("miao chong sheng").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c and c.Parent then
		local hum = c:FindFirstChildOfClass("Humanoid")
		if hum then
...
local loopgb = makecharbutton("lian xu miao chong sheng")
local loopg = false
spawn(function()
	while wait() and gui do
		if loopg then
...
loopgb.MouseButton1Click:Connect(function()
	loopg = not loopg
	loopgb.Text = "lian xu miao chong sheng" .. ((loopg and " (开)") or "")
end)
local flyb = makecharbutton("fei xing")
flyb.MouseButton1Click:Connect(function()
	if cfly and cfly.Parent then
		cfly = nil
	else
		cfly = lp.Character
		flyb.Text = "feng xing(kai)"
	end
end)
...
local hiddenflingB = makecharbutton("yin mi de tou zhi")
hiddenflingB.MouseButton1Click:Connect(function()
    hiddenfling = not hiddenfling
    hiddenflingB.Text = "yin mi de tou zhi" .. ((hiddenfling and "(kai)") or "")
end)

makecharbutton("fan chong sheng(deng dai 5 miao)").MouseButton1Click:Connect(respawnRequest)
makecharbutton("wu dong hua").MouseButton1Click:Connect(noanimations)
makecharbutton("zuo").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c and c.Parent then
		local hum = c:FindFirstChildOfClass("Humanoid")
		if hum then
...
makecharbutton("jia zuo").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c and c.Parent then
		local hum = c:FindFirstChildOfClass("Humanoid")
		if hum then
...
makecharbutton("yi chu ren xing ji qi ren").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c then
		local cpar = c.Parent
		if cpar then
...
makecharbutton("ke chu peng she zhi wei jia").MouseButton1Click:Connect(function()
	local c = lp.Character
	if c and c.Parent then
		local tool = c:FindFirstChildOfClass("Tool")
		for i, v in pairs(c:GetDescendants()) do
...
local ctrltp = false
local clicktpbutton = makecharbutton("an zhu CRTL shi dian ji chuan song")
clicktpbutton.MouseButton1Click:Connect(function()
    ctrltp = not ctrltp
    clicktpbutton.Text = "an zhu CRTL shi dain ji chuan song" .. ((ctrltp and " (kai)") or "")
end)
local con = nil
...
makecharbutton("jian shi").MouseButton1Click:Connect(unview)

local utilframe = makeFrame(scrollingFrame, "hui zhi", Color3.fromRGB(0, 150, 150))
...
local netb = makeutilbutton("wang luo pang lu")
local net = false
netb.MouseButton1Click:Connect(function()
	...
	netb.Text = "wang luo pang lu" .. ((net and "(kai)") or "")
end)

local espb = makeutilbutton("xian shi mo xing")
local esp = false
espb.MouseButton1Click:Connect(function()
	esp = not esp
	espb.Text = "xian shi mo xing" .. ((esp and " (kai)") or "")
end)
local teamcheckb = makeutilbutton("ESP tuan dui jian cha")
local espTeamCheck = false
teamcheckb.MouseButton1Click:Connect(function()
	espTeamCheck = not espTeamCheck
	teamcheckb.Text = "ESP tuan dui jian cha" .. ((espTeamCheck and " (kai)") or "")
end)
...
makeutilbutton("jia ru").MouseButton1Click:Connect(function()
...
makeutilbutton("fu wu qi yue diao").MouseButton1Click:Connect(function()
	local servers = nil
	pcall(function()
...
makeutilbutton("ji shi xiu jia").MouseButton1Click:Connect(function()
	lp:Kick()
	wait()
	game:Shutdown()
end)
...
local cbringframe = makeFrame(scrollingFrame, "chuan song bie ren", Color3.fromRGB(15, 100, 15))
...
cbringallbtn.Text = "+tian jia quan bu"
